...
# 🔥 V2.11.23: 從共用資料檔匯入，解決循環引用並統一數值設定
...
try:
    ...
    print(f"DB Init Warning (Friendship): {e}")
...
def get_wild_list(level: int, current_user: User = Depends(get_current_user)):
    ...
    if not unique_species: unique_species.add("小拉達") # 防呆
    ...
    for name in unique_species:
        ...
        wild_list.append({ 
            ...
            "is_powerful": False, 
            "level": level, "hp": wild_hp, "max_hp": wild_hp, 
            "attack": wild_atk, "image_url": base["img"], "skills": wild_skills 
        })
    return wild_list
...
async def wild_attack_api(is_win: bool = Query(...), is_powerful: bool = Query(False), target_name: str = Query("野怪"), target_level: int = Query(1), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    update_user_activity(current_user.id)
    current_user.hp = current_user.max_hp # 戰鬥結束自動回血
    
    if is_win:
        # 正規化名稱 (移除前綴)
        real_target_name = target_name.replace("🔥 ", "").replace("強大的 ", "").replace("✨ ", "").strip()
        target_data = POKEDEX_DATA.get(real_target_name, POKEDEX_DATA.get("小拉達"))
...
async def box_action(action: str, pokemon_uid: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if action == "release":
        ...
        if target["name"] in ["急凍鳥", "火焰鳥", "閃電鳥", "超夢", "夢幻", "鳳王", "洛奇亞"]:
